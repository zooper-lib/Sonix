...
class TestDataGenerator {
  ...
  static Future<bool> _hasBasicTestFiles() async {
    final basicFiles = [
      '$generatedPath/test_mono_44100.wav',
      '$generatedPath/test_stereo_44100.wav',
      '$generatedPath/test_short.mp3',
  ...
  static Future<void> _generateEssentialValidAudioFiles() async {
    ...
    await _generateWavFile('test_mono_44100.wav', sampleRate: 44100, channels: 1, durationSeconds: 0.5);
    await _generateWavFile('test_stereo_44100.wav', sampleRate: 44100, channels: 2, durationSeconds: 0.5);

    // Generate minimal files for different sizes (much smaller than full suite)
    await _generateWavFile('test_short.wav', sampleRate: 44100, channels: 1, durationSeconds: 0.1);
    await _generateWavFile('test_medium.wav', sampleRate: 44100, channels: 2, durationSeconds: 1.0); // Reduced from 10s

    // Generate minimal synthetic files for other formats
    await _generateSyntheticAudioFile('test_short.mp3', format: 'mp3', durationSeconds: 0.5);
    await _generateSyntheticAudioFile('test_sample.flac', format: 'flac', durationSeconds: 0.5);
    await _generateSyntheticAudioFile('test_sample.ogg', format: 'ogg', durationSeconds: 0.5);
